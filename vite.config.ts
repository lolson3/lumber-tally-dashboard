...
export default defineConfig(({ mode }) => {
  ...
  const apiTarget = env.VITE_TALLY_API_BASE_URL || "http://127.0.0.1:7304";
  ...
  return {
    plugins: [react()],
    server: { port: dashboardPort, strictPort: true, allowedHosts, proxy },
    preview: { port: dashboardPort, strictPort: true, allowedHosts, proxy },
    test: {
      environment: "jsdom",
